...
if [ -f mysql-5.6.25.tar.gz ];then
	...
	make
	make install
	/usr/local/mysql/scropts/mysql_install_db --user=mysql --datadir=/usr/local/mysql/data/ --basedir=/usr/local/mysql
	chown -R root.mysql /usr/local/mysql
	chown -R mysql /usr/local/mysql/data
	/bin/cp -f /usr/local/mysql/support-files/mysql.server /etc/init.d/mysqld
	chmod +x /etc/inti.d/mysqld
	/bin/cp -f /usr/loca/mysql/support-files/my-default.cnf /etc/my.cnf
...
install_php(){
#安装php时没有指定启动哪些模块功能,如果有用户可以根据实际情况自行添加额外功能如--with-gd等
yum -y install gcc libxm12-devel
if [ -f mhash-0.9.9.9.tat.gz ];then
